      ...
      <Tabs defaultValue="editar" className="flex min-h-0 flex-1 flex-col px-4 py-3">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pb-28">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        ...
                        patchDraft((p) => ({
                          ...p,
                          providerId: value === 'none' ?           <TabsContent value="precios" className="mt-0 space-y-4">
            <Card className="border-white/10 bg-[#1A1826]/90">
              <CardContent className="space-y-4 p-4">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      patchDraft((p) => ({
                        ...
                        extended: {
                          ...p.extended!,
                          purchaseConversionUnitPurchasePrice: Number(e.target.value.replace(',', '.')) || 0,
                        },
                      }))
                    }
                  />
          ...
          <TabsContent value="calc" className="mt-0 space-y-4">
            <Card className="border-white/10 bg-[#1A1826]/90">
              <CardContent className="space-y-4 p-4">
                <p className="text-sm text-slate-400">Calcula valor neto + IGV a partir del costo neto y un margen de utilidad objetivo sobre el costo.</p>
                <div className="grid gap-3 md:grid-cols-3">
                  ...
                  <table className="w-full text-left text-xs">
                    <thead className="bg-white/[0.04] text-slate-400">
                      <tr>
                        ...
                        <th className="p-2 max-w-[180px]">Anterior</th><th className="p-2 min-w-[240px]">Posterior</th><th className="p-2">Responsable</th>
                      </tr>
                    </thead>
                    <tbody>
                ...
                <Input
                  ...
                  onChange={(event) => {
                    ...
                    void Promise.all(jobs).then(() =>
                      patchDraft((p) => ({ ...p, extended: { ...p.extended!, galleryDataUrls: urls } })),
                    );
                  }}
                />
                <div className="grid gap-3 sm:grid-cols-3">
                  {(ex.galleryDataUrls ?? []).map((src, idx) => (
                    <div key={`g-${idx}-${String(src.length)}`} className="group relative rounded-lg border border-white/10">
                      <img src={src} alt="" className="h-40 w-full rounded object-cover bg-black/30" />
                      <Button type="button" size="icon" variant="destructive" className="absolute right-2 top-2 opacity-0 transition-opacity group-hover:opacity-100" onClick={() => patchDraft((p) => { const g = [...(p.extended!.galleryDataUrls ?? [])]; g.splice(idx, 1); return { ...p, extended: { ...p.extended!, galleryDataUrls: g } }; })}>
                      ...
                      {paginateSlice(lotRows, lPage).length === 0 ? (
                        ...
                        paginateSlice(lotRows, lPage).map((row) => (
                          <tr key={row.id} className="border-t border-white/5">
                            ...
                            <td className="p-2">
                              <Button type="button" size="sm" variant="ghost" className="text-red-400" onClick={() =>
                                patchDraft((p) => ({ ...p, extended: { ...p.extended!, lots: (p.extended!.lots ?? []).filter((x) => x.id !== row.id) } }))}>Quitar</Button>
                            </td>
                          </tr>
                        ))
                      )}
