      ...
      {showSectionLabels && (
        <div className="px-2.5 pb-2 pt-1">
          <div className="relative flex items-center group">
            <Search className="absolute left-3 w-3.5 h-3.5 text-slate-400 dark:text-slate-400 group-focus-within:text-cyan-500 transition-colors pointer-events-none" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Buscar en el menú..."
              className="w-full pl-8 pr-7 py-2 text-xs rounded-xl bg-white/90 dark:bg-slate-900/90 border border-slate-300 dark:border-slate-700/80 text-slate-800 dark:text-slate-100 placeholder:text-slate-400 shadow-xs hover:border-cyan-500/50 dark:hover:border-cyan-400/50 focus:border-cyan-500 dark:focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 dark:focus:ring-cyan-400/25 transition-all duration-200"
            />
            {searchQuery && (
              <button
                type="button"
                onClick={() => setSearchQuery('')}
                className="absolute right-2.5 p-0.5 rounded-full text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 hover:bg-slate-200/50 dark:hover:bg-slate-700/50 transition-all"
              >
